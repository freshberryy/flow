...
public class BoolValue extends Value {
    ...
    public int asInt(int line, int col) {
        throw new RuntimeError("타입 캐스팅 오류: bool 타입을 int로 캐스팅할 수 없습니다.", line, col);
    }
    @Override
    public float asFloat(int line, int col) {
        throw new RuntimeError("타입 캐스팅 오류: bool 타입을 float로 캐스팅할 수 없습니다.", line, col);
    }
    @Override
    ...
    public Value performUnaryOperation(String operator, int line, int col) {
        switch (operator) {
            ...
            case "-":
            case "+":
                throw new RuntimeError("타입 불일치 오류: bool 타입에 산술 연산을 할 수 없습니다.", line, col);
        }
