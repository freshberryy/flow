...
public class IntValue extends Value {
    ...
    @Override
    public int asInt(int line, int col) { return getValue(); }
    @Override
    public float asFloat(int line, int col) { return (float) getValue(); }
    @Override
        ...
        if (other.isInt() || other.isFloat()) {
            switch (operator) {
                ...
                case "%":
                    ...
                    return new FloatValue(getValue() % other.asFloat(line, col)); 
                case "==":
                    if (other.isInt()) return new BoolValue(Objects.equals(getValue(), other.getValue()));
                    return new BoolValue(getValue() == other.asFloat(line, col));
                case "!=":
                    if (other.isInt()) return new BoolValue(!Objects.equals(getValue(), other.getValue()));
                    return new BoolValue(getValue() != other.asFloat(line, col));
                case "<":
                    if (other.isInt()) return new BoolValue(getValue() < other.asInt(line, col));
                    return new BoolValue(getValue() < other.asFloat(line, col));
                case ">":
                    if (other.isInt()) return new BoolValue(getValue() > other.asInt(line, col));
                    return new BoolValue(getValue() > other.asFloat(line, col));
                ...
                case "&&":
                case "||":
                    throw new RuntimeError("타입 불일치 오류: int 타입은 논리 연산을 할 수 없습니다.", line, col);
            }
        }
    ...
    public Value performUnaryOperation(String operator, int line, int col) {
        switch (operator) {
            ...
            case "+": 
                return this;
            case "!":
                throw new RuntimeError("타입 불일치 오류: int 타입에 논리 NOT 연산을 할 수 없습니다.", line, col);
        }
